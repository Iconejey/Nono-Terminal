...
const slash_commands = [
  ...
  { name: "/open", description: "Open a file in the inline editor" },
  {
    name: "/add-pin",
    description:
      "Pin a directory to bookmarks (defaults to current directory if no path specified)",
  },
  {
    name: "/pins",
    description:
      "Switch to a pinned directory (autocompletes pinned directories)",
  },
  {
    name: "/unpin",
...
function renderSuggestions(filtered) {
  ...
  filtered.forEach((cmd, idx) => {
    ...
    const colorStyle = cmd.gitStatusColor
      ? `style="color: ${cmd.gitStatusColor} !important;"`
      : "";
    item.innerHTML = `
...
function submitInput(text, usePro = false) {
  ...
  /test-md        - Simulate AI responding with markdown-debug-example.md content`;
      ...
      const match = pinned_dirs_global.find((dir) => {
        ...
        return (
          dir_name.toLowerCase() === clean_arg.toLowerCase() ||
          dir.toLowerCase() === clean_arg.toLowerCase()
        );
      });
      ...
      if (is_mobile) {
        ...
        if (!document.fullscreenElement && !document.webkitFullscreenElement) {
          ...
          if (enterFS) {
            ...
            out_pre.textContent =
              "Fullscreen is not supported on this mobile device/browser.";
          }
        } else {
          const exitFS =
      ...
      if (!is_mobile) {
        ...
        out_pre.className = "output";
        out_pre.textContent =
          "Error: The /screen command is only available on mobile devices connection.";
        active_block.appendChild(out_pre);
        appendNewPromptBlock(current_cwd);
        return;
      }
    ...
    container.addEventListener("touchend", (e) => {
      ...
      if (isDragging) {
        ...
        const moveDistance = Math.sqrt(
          Math.pow(endTouchX - startTouchX, 2) +
...
if (!is_mobile) {
  ...
  if (window.api.onStartScreenStream) {
    window.api.onStartScreenStream(async ({ socketId }) => {
      console.log(
        "Desktop WebRTC: received start-screen-stream for socket:",
        socketId,
      );
      try {
        ...
        if (!localScreenStream) {
          ...
          console.log(
            `Desktop WebRTC: Capturing display at native resolution ${screenW}x${screenH}`,
          );
...
window.api.onAgentToolStart((info) => {
  ...
  if (info.name === "execute_command") {
    ...
    else if (info.name === "web_search")
      label = `Searching the web for "${info.args.query}"`;
...
function handlePinsSuggestions(query, cmdPrefix = "/pins") {
  const matches = pinned_dirs_global.filter((dir) => {
    ...
    const dir_name = parts.pop() || parts.pop() || dir;
    return (
      dir_name.toLowerCase().includes(query.toLowerCase()) ||
...
async function saveEditorContent() {
  ...
  saveBtn.innerHTML =
    '<span class="material-symbols-outlined" style="font-size: 1.2em;">sync</span> <span class="btn-text">Saving...</span>';
  ...
  if (result.error) {
    saveBtn.innerHTML =
      '<span class="material-symbols-outlined" style="font-size: 1.2em;">error</span> <span class="btn-text">Error</span>';
    ...
    saveBtn.innerHTML =
      '<span class="material-symbols-outlined" style="font-size: 1.2em;">done</span> <span class="btn-text">Saved</span>';
...
async function updateScrollbarDecorations() {
  ...
  if (editor_mode === "diff") {
    ...
    if (!diffRaw || diffRaw.trim() === "") {
      ...
      const origResult = await window.api.readFileContent(
        active_editor_file_path,
      );
      ...
      if (hunkStartIndex !== -1) {
        const match = lines[hunkStartIndex].match(
          /^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@/,
        );
        if (match) {
